...
# def main():
#     st.set_page_config(page_title="Janashakthi Life Insurance Proposal", layout="wide")
...
#     # Create Tabs for the massive form
...
#     # TAB 5: Medical Questionnaire
#     with tab5:
#         st.header("12. Medical History")
#         st.markdown("Check 'Yes' if applicable to Main Life, Spouse, or Children.")
...
#         Before saving to AWS S3, the system will use OCR and AI to verify that the uploaded
#         documents match the details provided in this proposal form.
#         """)
...
#             )
#             if id_card_file:
#                 st.success(f"Uploaded: {id_card_file.name} ({id_card_file.size} bytes)")
...
#                 st.success(f"Uploaded: {utility_bill_file.name} ({utility_bill_file.size} bytes)")
...
#             else:
#                 with st.spinner("Running OCR and AI Document Verification... (This may take 1-2 minutes)"):
#                     import requests
#                     try:
#                         files = {
#                             "id_card_image": (id_card_file.name, id_card_file.getvalue(), id_card_file.type),
#                             "utility_bill_image": (utility_bill_file.name, utility_bill_file.getvalue(), utility_bill_file.type),
#                         }
...
#                             st.error(ver_response.text)
#                             st.session_state.verification_result = None
#                     except requests.exceptions.ConnectionError:
#                         st.error("Could not connect to the backend API. Make sure the backend is running on port 8000.")
#                     except requests.exceptions.Timeout:
#                         st.error("Verification timed out. The OCR + LLM process took too long.")

#         # Display verification results if they exist in state
...
#             # Show raw OCR outputs for debugging
...
#                 else:
#                     if failed_fields:
#                         st.error(f"❌ Cannot submit. Score is {score}% (minimum 80% required). The following fields did NOT match: **{', '.join(failed_fields)}**. Please check your uploaded documents and the proposal form details.")
#                     else:
